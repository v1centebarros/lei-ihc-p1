export const fullData = [{
    id: 0,
    name: "Rádio Comercial",
    icon: "http://radiocomercial.iol.pt/images/noCover.png",
    banner: "https://images.rfm.sapo.pt/cafe_da_manha_2020_fb59699d93.png",
    liveNow: "Que barulho é este?",
    programs: [{
            id: 0,
            name: "Era o que faltava",
            description: "Sabe quando as conversas chegam àquela zona? Lembra-se da última vez que pensou 'eu não devia estar a falar sobre isto'? Na Rádio Comercial, essas conversas têm hora marcada. Era o que Faltava, de segunda a sexta, às 20h. Com João Paulo Sousa e Ana Delgado Martins.",
            icon: "https://radiocomercial.iol.pt/upload/E/era-o-que-faltava-radio-comercial-square.webp",
            banner: "https://radiocomercial.iol.pt/upload/E/era-o-que-faltava-radio-comercial-square.webp",
            genres: [
                "Talk Show",
            ],
            episodes: [{
                    id: 0,
                    name: "Lurder Beata",
                    description: "Com quase 30 anos de experiência como jornalista, Lurdes Baeta veio ao Era o Que Faltava contar a experiência que viveu nas semanas que esteve em trabalho na Ucrânia.",
                    date: "30/05/2022",
                    time: "40"
                },
                {
                    id: 1,
                    name: "José Pacheco Pereira",
                    description: "Participou na luta contra a ditadura, é um nome incontornável quando falamos de história e investigação em Portugal e veio ao Era ao Que Faltava desfiar memórias sobre uma vida carregada de memórias. José Pacheco Pereira, ou simplesmente Pacheco Pereira, confessa que é possível salvar o mundo e revela a fórmula: \"Só se salva o mundo pela consciência da liberdade, da democracia e do bem comum\".",
                    date: "27/05/2022",
                    time: "65"
                },
            ],
        },
        {
            id: 1,
            name: "Hollywood Express",
            description: "O novo podcast da Comercial que lhe dá a ouvir as novidades do cinema e da televisão na 'óptica do utilizador'. É feito para quem gosta de filmes e séries por quem adora falar sobre isso. Há uma nova edição para ouvir sempre à sexta-feira.",
            icon: "https://radiocomercial.iol.pt/upload/H/HOLLYWOOD_EXPRESS_1200-square.webp",
            banner: "https://radiocomercial.iol.pt/upload/H/HOLLYWOOD_EXPRESS_1200.png",
            genres: [
                "Films",
                "Culture",
            ],
            episodes: [{
                    id: 0,
                    name: "NUNO MARKL ANALISA \"DOUTOR ESTRANHO NO MULTIVERSO DA LOUCURA\" ",
                    description: "O Nuno Markl regressa ao Hollywood Express para falar do filme da semana da Rádio Comercial. Fã da Marvel, o nosso Nuno já foi ver 'Doutor Estranho no Multiverso da Loucura' e vai contar-lhe o que pode sobre a nova incursão de Sam Raimi na Marvel.",
                    date: "06/05/2022",
                    time: "24"
                },
                {
                    id: 1,
                    name: "'O HOMEM DO NORTE': ENTREVISTAS A ALEXANDER SKARSGARD E A ROBERT EGGERS",
                    description: "\"O Homem do Norte\" é o filme da semana da Rádio Comercial e a Patrícia Pereira esteve à conversa com o realizador desta saga Viking e com o protagonista.",
                    date: "22/04/2022",
                    time: "33"
                }
            ],
        },
    ],
    chat: [{
        "title": 2,
        "name": "Pò",
        "message": "Decentralized static installation"
    }, {
        "title": 1,
        "name": "Pénélope",
        "message": "Horizontal zero tolerance product"
    }, {
        "title": 3,
        "name": "Nélie",
        "message": "Reactive object-oriented circuit"
    }, {
        "title": 3,
        "name": "Jú",
        "message": "Organized optimal ability"
    }, {
        "title": 2,
        "name": "Océane",
        "message": "Balanced bottom-line access"
    }, {
        "title": 2,
        "name": "Laurène",
        "message": "Persevering reciprocal Graphic Interface"
    },{
        "title": 3,
        "name": "Lucrèce",
        "message": "Balanced object-oriented function"
    }, {
        "title": 2,
        "name": "Yóu",
        "message": "Re-engineered zero defect support"
    }, {
        "title": 2,
        "name": "Eugénie",
        "message": "Visionary asynchronous policy"
    }]
},

//coiso 2
{
    id: 1,
    name: "RFM",
    icon: "https://play-lh.googleusercontent.com/MhpmqORmqFYvYLYKV2hWwcKz9Ned_l1Mjznbm0G5YVETlXfa0rj3gnjPP30ZtV3isSg",
    banner: "https://images.rfm.sapo.pt/cafe_da_manha_2020_fb59699d93.png",
    liveNow: "Que barulho é este?",
    programs: [{
            id: 0,
            name: "STAND-UP NA HORA",
            description: "Sem leituras nem guiões, e apenas com a ajuda das notas que vai apontando no telemóvel, Salvador vai fazer Stand-up na hora. Sem beber bagaço. Quer dizer, ainda é cedo para falar. Um dia de cada vez.",
            icon: "https://cdn.popcasts.pt/wp-content/uploads/2021/09/image-1628158532-2048x2048.jpg",
            banner: "https://images.rfm.sapo.pt/stand_up_na_hora_socialshare66244cf4.jpg",
            genres: [
                "Comedy",
                "Pop",
            ],
            episodes: [{
                    id: 0,
                    name: "António Costa brinca o rei manda.",
                    description: "A abstenção desceu! A análise de Salvador.",
                    date: "31/01/2022",
                    time: "5"
                },
                {
                    id: 1,
                    name: "7 perguntas a Fernando Daniel",
                    description: "Aproveitando a presença do Fernando Daniel em estúdio, 7 perguntas.",
                    date: "28/01/2022",
                    time: "4"
                },
                {
                    id: 2,
                    name: "Pessoas demasiado à vontade",
                    description: "A pedido de ouvinte Salvador lançou o tema. Não toque no queijo!",
                    date: "27/01/2022",
                    time: "4"
                }
            ],
        },
        {
            id: 1,
            name: "CARA PODRE (Videocast)",
            description: "Cara Podre faz parte do programa Wi-Fi na RFM conduzido por Joana Cruz, Rodrigo Gomes e Daniel Fontoura. São perguntas à Cara Podre que os convidados têm de responder à Cara Podre.",
            icon: "https://cdn.popcasts.pt/wp-content/uploads/2022/04/rfm-cara-podre35215742_quadrada_agenda_top25_locutores_autopromo_capaPlayer.jpg",
            banner: "https://images.rfm.sapo.pt/cara_podre_rita_pere1725ea53_destaque_2col.jpg",
            genres: [
                "Comedy",
                "Interview"
            ],
            episodes: [{
                    id: 0,
                    name: "Cara Podre com Fernando Rocha (Videocast)",
                    description: "O Fernando Rocha jogou ao Cara Podre no Wi-Fi da RFM com Joana Cruz, Rodrigo Gomes e Daniel Fontoura",
                    date: "22/04/2022",
                    time: "6"
                },
                {
                    id: 1,
                    name: "Cara Podre com Susana Vieira (Videocast)",
                    description: "A Susana Vieira jogou ao Cara Podre no Wi-Fi da RFM com Joana Cruz, Rodrigo Gomes e Daniel Fontoura",
                    date: "21/04/2022",
                    time: "7"
                }
            ],
        },

        {
            id: 2,
            name: "ORA BOLAS",
            description: "As grandes histórias do mundo do ténis, para ouvir no podcast oficial do Millennium Estoril Open.",
            icon: "https://cdn.popcasts.pt/wp-content/uploads/2022/04/image-1650553160-2048x2048.jpg",
            banner: "https://cdn.popcasts.pt/wp-content/uploads/2022/04/image-1650553160-2048x2048.jpg",
            genres: [
                "Sports"
            ],
            episodes: [{
                    id: 0,
                    name: "Ora Bolas ep9 Caça ao brinde",
                    description: "Durante 9 dias há animação no espaço da Smash Area. É aqui que se pode descansar entre jogos e ganhar brindes ou fazer algumas atividades, Desde conduzir um Porsche em simulador, ganhar bilhetes para os encontros, porta chaves, protetores solares, relógios e muito mais!",
                    date: "01/05/2022",
                    time: "20"
                },
                {
                    id: 1,
                    name: "Ora Bolas ep8 segredos que se partilham",
                    description: "Pedro Keul e Miguel Seabra, os convidados do Ora Bolas de hoje! As histórias das estrelas que não jogam nos courts mas fazem muitos ases para que tudo corra bem.",
                    date: "30/04/2022",
                    time: "10"
                },
                {
                    id: 2,
                    name: "Ora Bolas ep7 Wi-Fi da RFM em campo",
                    description: "No sétimo episódio do Ora Bolas! fomos até à zona VIP. O Wi-Fi da RFM é feito ao vivo, a partir do Slice Lounge, durante 3 noites. A primeira foi assim: Joana Crz, Rodrigo Gomes e Daniel Fontoura receberam figuras públicas como Sebastien Coates, Guilhaume, Tiago Bettencourt e Noble",
                    date: "29/04/2022",
                    time: "90"
                }


            ],
        },

    ],
    chat: [{
        "title": 2,
        "name": "Pò",
        "message": "Decentralized static installation"
    }, {
        "title": 1,
        "name": "Pénélope",
        "message": "Horizontal zero tolerance product"
    }, {
        "title": 3,
        "name": "Nélie",
        "message": "Reactive object-oriented circuit"
    }, {
        "title": 3,
        "name": "Jú",
        "message": "Organized optimal ability"
    }, {
        "title": 2,
        "name": "Océane",
        "message": "Balanced bottom-line access"
    }, {
        "title": 2,
        "name": "Laurène",
        "message": "Persevering reciprocal Graphic Interface"
    }, {
        "title": 3,
        "name": "Bécassine",
        "message": "Focused contextually-based orchestration"
    }, {
        "title": 3,
        "name": "Lén",
        "message": "Persevering interactive hub"
    }, {
        "title": 1,
        "name": "Thérèse",
        "message": "Diverse 3rd generation orchestration"
    }, {
        "title": 3,
        "name": "Salomé",
        "message": "Versatile encompassing website"
    }, {
        "title": 2,
        "name": "Hélène",
        "message": "Public-key context-sensitive matrix"
    }, {
        "title": 2,
        "name": "Sòng",
        "message": "Advanced attitude-oriented policy"
    }, {
        "title": 1,
        "name": "Salomé",
        "message": "Expanded national superstructure"
    }, {
        "title": 2,
        "name": "Fèi",
        "message": "Fundamental tertiary hierarchy"
    }, {
        "title": 1,
        "name": "Lén",
        "message": "Inverse real-time benchmark"
    }, {
        "title": 1,
        "name": "Almérinda",
        "message": "Customizable didactic open system"
    }, {
        "title": 3,
        "name": "Lucrèce",
        "message": "Balanced object-oriented function"
    }, {
        "title": 2,
        "name": "Yóu",
        "message": "Re-engineered zero defect support"
    }, {
        "title": 2,
        "name": "Eugénie",
        "message": "Visionary asynchronous policy"
    }]
},

//coiso 3
{
    id: 2,
    name: "CidadeFM",
    icon: "https://cidade.iol.pt/images/noCover.png",
    banner: "https://images.rfm.sapo.pt/cafe_da_manha_2020_fb59699d93.png",
    liveNow: "Que barulho é este?",
    programs: [{
        id: 0,
        name: "Já são horas",
        description: "Um programa que é tudo mas que, ao mesmo tempo, não é nada. Não tem dias maus mas uma coisa é garantida, dias bons não vão faltar. Não é intenso mas também não é murcho. É cedo, isso sim! Depois do barulho mais doloroso do mundo, o do despertador, liga a Cidade FM que a Laura Ferreira e o Rui Simões ajudam-te a sair da cama. Ou não... só se quiseres. Das 7h às 11h, Já São Horas de te ligares à Cidade FM, a rádio jovem mais ouvida em Portugal.",
        icon: "https://cidade.iol.pt/upload/J/jsh_lr_-_copy.jpg",
        banner: "https://cidade.iol.pt/upload/S/site_jsh.png",
        genres: [
            "Comedy",
            "Pop",
        ],
        episodes: [{
                id: 0,
                name: "Carlos - Café de Bairro",
                description: "O Carlos é o autor de uma página de Instagram com mais de 100k seguidores - Café de Bairro- e a sua identidade é desconhecida. Hoje apresentou a sua voz pela primeira vez na rádio, com a Laura Ferreira e o Rui Simões.",
                date: "27/05/2022",
                time: "16"
            },
            {
                id: 1,
                name: "David Carreira",
                description: "Noite Inteira é o novo single do David Carreira que vai animar o teu verão!",
                date: "08/04/2022",
                time: "26"
            }
        ],
    },

    {
        id: 1,
        name: "Helder Tavares",
        description: "Seja a que hora do dia for, é sempre o mais animado da sala! Nunca se cala mas vai tentar dar-te a melhor música sem interromper! Para além de Youtuber e Influencer, também tem a mania que é chef de cozinha e às vezes... locutor de rádio!",
        icon: "https://cidade.iol.pt/upload/H/helder_tavares3.png",
        banner: "https://cidade.iol.pt/upload/S/site_h1-social.png",
        genres: [
            "Comedy",
            "Pop",
        ],
        episodes: [{
                id: 0,
                name: "Bruna Guedelha",
                description: "Cabelo Pantene - O Sonho 4: A Bruna Guedelha foi a grande vencedora do Cabelo Pantene - O Sonho 2!",
                date: "20/04/2022",
                time: "16"
            },
            {
                id: 1,
                name: "Asizé",
                description:"Cabelo Pantene - O Sonho 4: A Asizé foi a grande vencedora do Cabelo Pantene - O Sonho 3!",
                date: "13/04/2022",
                time: "15"
            },
            {
                id: 2,
                name: "Sofia Ribeiro e Luís Borges",
                description:"Cabelo Pantene - O Sonho 4: A Sofia Ribeiro e o Luís Borges vão ser os apresentadores da nova temporada!",
                date: "08/04/2022",
                time: "43"
            }
        ],
    }
],
    chat: [{
        "title": 2,
        "name": "Pò",
        "message": "Decentralized static installation"
    }, {
        "title": 1,
        "name": "Pénélope",
        "message": "Horizontal zero tolerance product"
    }, {
        "title": 3,
        "name": "Nélie",
        "message": "Reactive object-oriented circuit"
    }, {
        "title": 3,
        "name": "Jú",
        "message": "Organized optimal ability"
    }, {
        "title": 2,
        "name": "Océane",
        "message": "Balanced bottom-line access"
    }, {
        "title": 2,
        "name": "Laurène",
        "message": "Persevering reciprocal Graphic Interface"
    }, {
        "title": 3,
        "name": "Bécassine",
        "message": "Focused contextually-based orchestration"
    }, {
        "title": 3,
        "name": "Lén",
        "message": "Persevering interactive hub"
    }, {
        "title": 1,
        "name": "Thérèse",
        "message": "Diverse 3rd generation orchestration"
    }, {
        "title": 3,
        "name": "Salomé",
        "message": "Versatile encompassing website"
    }, {
        "title": 2,
        "name": "Hélène",
        "message": "Public-key context-sensitive matrix"
    }, {
        "title": 2,
        "name": "Sòng",
        "message": "Advanced attitude-oriented policy"
    }, {
        "title": 1,
        "name": "Salomé",
        "message": "Expanded national superstructure"
    }, {
        "title": 2,
        "name": "Fèi",
        "message": "Fundamental tertiary hierarchy"
    }, {
        "title": 1,
        "name": "Lén",
        "message": "Inverse real-time benchmark"
    }, {
        "title": 1,
        "name": "Almérinda",
        "message": "Customizable didactic open system"
    }, {
        "title": 3,
        "name": "Lucrèce",
        "message": "Balanced object-oriented function"
    }, {
        "title": 2,
        "name": "Yóu",
        "message": "Re-engineered zero defect support"
    }, {
        "title": 2,
        "name": "Eugénie",
        "message": "Visionary asynchronous policy"
    }]
},

//coiso 4

{
    id: 3,
    name: "MegaHits",
    icon: "https://cdn.popcasts.pt/wp-content/uploads/2021/04/MEGA-HITS.jpg",
    banner: "https://images.rfm.sapo.pt/cafe_da_manha_2020_fb59699d93.png",
    liveNow: "Bota Fora",
    programs: [{
            id: 0,
            name: "Se fosse a ti não atendia!!",
            description: "Cuidado com as chamadas que recebes, a partir de agora! Pode bem ser a JOANA SEQUEIRA… e de repente, estás a ser apanhado! Para ouvir no SNOOZE às segundas e sextas, depois das 8 da manhã.",
            icon: "https://cdn.popcasts.pt/wp-content/uploads/2022/05/image-1652892426-2048x2048.jpg",
            banner: "https://images.megahits.sapo.pt/sfatna-1600x4503913c72c_hwide.jpg",
            genres: [
                "Comedy",
                "Interview",
            ],
            episodes: [{
                    id: 0,
                    name: "Ep.4 | Aulas de Francês em Campos de Lavanda",
                    description: "Será que a Carolina Carvalho aceitou o convite da Joana Sequeira para aprender francês enquanto faz a posição do cão?",
                    date: "27/05/2022",
                    time: "3"
                },
                {
                    id: 1,
                    name: "Ep.3 | Bombeiro Maroto",
                    description: "Foi prank call, mas não seria assim tão improvável que o Salvador Martinha tivesse sugerido ao Luís Franco Bastos para ir a uma festa vestido de bombeiro maroto…",
                    date: "23/05/2022",
                    time: "1"
                },
                {
                    id: 2,
                    name: "Ep.2 | Ladra de hotéis",
                    description: "Já levaste alguma coisa de um hotel? Champô, cremes, toalhas, almofadas, etc… Será que a Catarina Palma também o fez?",
                    date: "20/05/2022",
                    time: "3"
                }

            ],
        },
        {
            id: 1,
            name: "Cala-te Boca!",
            description: "Ninguém disse que ir ao Snooze era fácil…Será que os convidados reagem bem quando lhes fazem as perguntas proibidas? Só podem passar uma vez… depois disso confessam tudo ao Conguito, à Mafalda e à Teresa. É assim o CALA-TE BOCA!",
            icon: "https://cdn.popcasts.pt/wp-content/uploads/2021/09/image-1627634995-2048x2048.jpg",
            banner: "https://images.megahits.sapo.pt/mega_hits_cala_te_boca_1920x10807339d9a9_socialshare.jpg",
            genres: [
                "Comedy",
            ],
            episodes: [{
                    id: 0,
                    name: "David Carreira e Carolina Carvalho",
                    description: "O David e a Carolina brindaram-nos com a maior energia de sempre toda a manhã. Nós, em troca, fizemos um CALA-TE BOCA imperdível! Será que se safaram?",
                    date: "27/05/2022",
                    time: "8"
                },
                {
                    id: 1,
                    name: "Clara Não",
                    description: "A Clara Não, para além de ilustradora, designer e artista, define-se como uma rebelde por dizer tudo o que pensa. Será que levou esta filosofia para o CALA-TE BOCA e nos disse tudiiinho?",
                    date: "24/05/2022",
                    time: "8"
                },
                {
                    id: 2,
                    name: "Vicky Jacobi",
                    description: "A mais nova das irmãs Patrocínio e a única considerada uma possível rival de Carolina Patrocínio em termos de comunicação e à vontade. No meio de dancinhas, trends e desafios, a Vicky já conseguiu 80 mil seguidores no TikTok. Será que lançou o trunfo do CALA-TE BOCA? Ouve.",
                    date: "19/05/2022",
                    time: "9"
                },
                {
                    id: 3,
                    name: "Merche Romero",
                    description: "Cala-te Boca com Merche Romero não seria Cala-te Boca se não tocássemos no assunto Cristiano Ronaldo! Ficámos a saber que Ronaldo é uma pessoa cheia de sentido de humor mas ela também nos contou algumas das suas manias! Ora ouve.",
                    date: "07/04/2022",
                    time: "10"
                },
                {
                    id: 4,
                    name: "Ana Barbosa",
                    description: "No Big Brother 2021 mostrou a capitã que há em si e trouxe a vitória para casa. Ana Barbosa é uma entertainer nata com resposta sempre na ponta da língua, mas será que se safa ao desafio do “Cala-te Boca”?",
                    date: "23/03/2022",
                    time: "9"
                }
            ],
        },

    ],
    chat: [{
        "title": 2,
        "name": "Pò",
        "message": "Decentralized static installation"
    }, {
        "title": 1,
        "name": "Pénélope",
        "message": "Horizontal zero tolerance product"
    }, {
        "title": 3,
        "name": "Nélie",
        "message": "Reactive object-oriented circuit"
    }, {
        "title": 3,
        "name": "Jú",
        "message": "Organized optimal ability"
    }, {
        "title": 2,
        "name": "Océane",
        "message": "Balanced bottom-line access"
    }, {
        "title": 2,
        "name": "Laurène",
        "message": "Persevering reciprocal Graphic Interface"
    }, {
        "title": 3,
        "name": "Bécassine",
        "message": "Focused contextually-based orchestration"
    }, {
        "title": 3,
        "name": "Lén",
        "message": "Persevering interactive hub"
    }, {
        "title": 1,
        "name": "Thérèse",
        "message": "Diverse 3rd generation orchestration"
    }, {
        "title": 3,
        "name": "Salomé",
        "message": "Versatile encompassing website"
    }, {
        "title": 2,
        "name": "Hélène",
        "message": "Public-key context-sensitive matrix"
    }, {
        "title": 2,
        "name": "Sòng",
        "message": "Advanced attitude-oriented policy"
    }]
},

//coiso 5

{
    id: 4,
    name: "Renascença",
    icon: "https://popcasts.pt/wp-content/uploads/2021/04/RENASCENCA.png",
    banner: "https://popcasts.pt/wp-content/uploads/2021/04/RENASCENCA.png",
    liveNow: "Que barulho é este?",
    programs: [{
            id: 0,
            name: "Hotel Califórnia",
            description: "Sábados, das 10h às 12h faça uma viagem musical aos anos 70 e 80 com Paulino Coelho e Júlio Isidro.Ao fim-de-semana opte por se instalar aqui: Hotel Califórnia.",
            icon: "https://cdn.popcasts.pt/wp-content/uploads/2022/05/image-1652110820-2048x2048.jpg",
            banner: "https://cdn.popcasts.pt/wp-content/uploads/2022/05/image-1652110820-2048x2048.jpg",
            genres: [
                "Music",
            ],
            episodes: [{
                    id: 0,
                    name: "Hotel Califórnia - 28 de Maio",
                    description: "Sábados, das 10h às 12h faça uma viagem musical aos anos 70 e 80 com Paulino Coelho e Júlio Isidro. Ao fim-de-semana opte por se instalar aqui: Hotel Califórnia.",
                    date: "28/05/2022",
                    time: "100"
                },
                {
                    id: 1,
                    name: "Hotel Califórnia - 21 de Maio",
                    description: "Sábados, das 10h às 12h faça uma viagem musical aos anos 70 e 80 com Paulino Coelho e Júlio Isidro. Ao fim-de-semana opte por se instalar aqui: Hotel Califórnia.",
                    date: "21/05/2022",
                    time: "99"
                },
                {
                    id: 2,
                    name: "Hotel Califórnia - 14 de Maio",
                    description: "Sábados, das 10h às 12h faça uma viagem musical aos anos 70 e 80 com Paulino Coelho e Júlio Isidro. Ao fim-de-semana opte por se instalar aqui: Hotel Califórnia.",
                    date: "14/05/2022",
                    time: "95"
                },
                {
                    id: 3,
                    name: "Hotel Califórnia - 7 de Maio",
                    description: "Sábados, das 10h às 12h faça uma viagem musical aos anos 70 e 80 com Paulino Coelho e Júlio Isidro. Ao fim-de-semana opte por se instalar aqui: Hotel Califórnia.",
                    date: "7/05/2022",
                    time: "101"
                }

            ],
        },
        {
            id: 1,
            name: "Compromisso Verde",
            description: "“Compromisso Verde” é um podcast quinzenal, da autoria de Cristina Nascimento, sobre crescimento sustentável e transição energética. Da pandemia dos plásticos aos pecados da chamada moda rápida, cada episódio um tema. Uma nova edição disponível às quartas-feiras às 18h00. Esta é uma uma parceria Renascença/Euranet Plus.",
            icon: "https://cdn.popcasts.pt/wp-content/uploads/2022/04/image-1648122275-2048x2048.jpg",
            banner: "https://cdn.popcasts.pt/wp-content/uploads/2022/04/image-1648122275-2048x2048.jpg",
            genres: [
                "Education",
            ],
            episodes: [{
                    id: 0,
                    name: "Futuro da agricultura e alterações climáticas. “Estamos a perder área de cultivo”",
                    description: "Associação dos Jovens Agricultores de Portugal mostra-se preocupada com os níveis de seca e a desertificação do país. Diretor-geral da associação acredita que a ciência e a inovação são fundamentais para ultrapassar fragilidades do setor.",
                    date: "25/05/2022",
                    time: "15"
                },
                {
                    id: 1,
                    name: "Políticos consideram “infantis” mobilizações dos jovens contra alterações climáticas",
                    description: "Especialista sobre participação política e ativismo ambiental defende urgência da aproximação dos decisores políticos aos jovens. Carla Malafaia diz que falta, no entanto, vontade política.",
                    date: "11/05/2022",
                    time: "17"
                } 
            ],
        },

        {
            id: 2,
            name: "Geração Z",
            description: "Nasceram na era das tecnologias de informação, são mais práticos e mobilizam-se por causas. Até dispensam o carro e a casa, também porque não têm grandes salários para pagá-los, mas arriscam ter o seu próprio negócio. Como podemos ajudá-los? Quais os medos que enfrentam? Que tal começarmos por ouvi-los?“Geração Z” é um podcast quinzenal, publicado à quarta-feira, às 18h, da autoria da jornalista Beatriz Lopes. Esta é uma uma parceria Renascença/Euranet Plus.",
            icon: "https://cdn.popcasts.pt/wp-content/uploads/2022/04/image-1648122234-2048x2048.jpg",
            banner: "https://cdn.popcasts.pt/wp-content/uploads/2022/04/image-1648122234-2048x2048.jpg",
            genres: [
                "News",
            ],
            episodes: [{
                    id: 0,
                    name: "Geração Z e o Futuro da Europa. “É preciso garantir que não temos uma geração perdida”",
                    description: "O futuro da Europa esteve em debate entre cidadãos e políticos ao longo do último ano. Os jovens também tiveram direito a um lugar na mesa redonda e participaram nas 49 propostas finais deixadas aos líderes europeus. O que dizem? E passarão mesmo da teoria para a prática?",
                    date: "18/05/2022",
                    time: "26"
                },
                {
                    id: 1,
                    name: "Geração Z e a migração. “Em Berlim, fiquei tão fã da comida vietnamita que era o meu almoço quase habitual”",
                    description: "Alexandre saiu de casa para a Hungria e seguiu para Chile, Alemanha, Estados Unidos e Espanha. Qual a relação da Geração Z com o mundo? Como vê a multiculturalidade?",
                    date: "04/05/2022",
                    time: "20"
                },
                {
                    id: 2,
                    name: "“É um presente envenenado. Somos a geração mais qualificada, mas quase que pagamos para trabalhar”",
                    description: "Passam a vida a ouvir “os políticos a apregoar” que são a geração mais qualificada de sempre, mas isso não se reflete na estabilidade laboral e nos salários. Pedem “condições à altura do talento português” que não os obrigue a emigrar ou a “sobreviver em Lisboa”. O diretor do Observatório das Desigualdades alerta: a distância entre a taxa de desemprego geral e a taxa de desemprego jovem “nunca foi tão grande” e “aproveitar o potencial dos mais jovens” deveria ser uma das grandes prioridades do novo governo.",
                    date: "20/04/2022",
                    time: "31"
                }  

            ],
        },

        {
            id: 3,
            name: "Economisses",
            description: "Às terças, no Turno da Tarde vai aprender ou relembrar formas simples de gerir o seu dinhiero e conseguir a sua liberdade e independência financeira.Susana Rosa é formada em Gestão e Marketing Digital, aprendeu, na prática, ao longo da vida a importância que uma boa gestão financeira tem no dia a dia e fundou o projeto EconoMisses.Economisses ganha agora um lugar na sua rádio. Não perca dicas preciosas todas as terças às 16h40.",
            icon: "https://cdn.popcasts.pt/wp-content/uploads/2022/04/image-1651066865-2048x2048.jpg",
            banner: "https://cdn.popcasts.pt/wp-content/uploads/2022/04/image-1651066865-2048x2048.jpg",
            genres: [
                "Entrepreneurship",
                "Business"
            ],
            episodes: [{
                    id: 0,
                    name: "Poupar quando se vai a um casamento",
                    description: "Esta a começar a época dos casamentos e quando temos muitos durante o verão, o orçamento familiar sofre. A Susana Rosa traz dicas para poupar quando se tem muitos convites para casamentos.",
                    date: "24/05/2022",
                    time: "6"
                },
                {
                    id: 1,
                    name: "O que fazer com o reembolso do IRS",
                    description: "Para algumas pessoas esta é uma forma de receber um dinheiro extra. Mas o que devemos fazer com esse dinheiro? A Susana Rosa dá algumas dicas.",
                    date: "17/05/2022",
                    time: "5"
                } 
            ],
        },
        {
            id: 4,
            name: "Extremamente Desagradável",
            description: "A rubrica de humor de “As Três da Manhã” está a cargo de Joana Marques, que se esforça sempre por ser “extremamente desagradável”, agora na Renascença. De segunda a sexta, às 8h15 com repetição às 16h20, 20h30 e às 6h45.",
            icon: "https://i.scdn.co/image/ab6765630000ba8a3c9f36d771c507a16572ffa3",
            banner: "https://gruporenascencamultimedia.com/wp-content/uploads/2021/12/extre-desagradavel_base-002.jpg",
            genres: [
                "Comedy",
            ],
            episodes: [{
                    id: 0,
                    name: "Rúben Semedo é Alerta CM",
                    description: "Joana Marques fala-nos da mais recente polémica envolvendo Ruben Semedo.",
                    date: "22/04/2022",
                    time: "14"
                },
                {
                    id: 1,
                    name: "O Último a Trair",
                    description: "Joana Marques fala-nos da atribulada lua de mel de Tiago e Dina, participantes do \"Casados à primeira vista\".",
                    date: "21/04/2022",
                    time: "15"
                }
            ],
        },
    ],
    chat: [{
        "title": 2,
        "name": "Pò",
        "message": "Decentralized static installation"
    }, {
        "title": 1,
        "name": "Pénélope",
        "message": "Horizontal zero tolerance product"
    }, {
        "title": 3,
        "name": "Nélie",
        "message": "Reactive object-oriented circuit"
    }, {
        "title": 3,
        "name": "Jú",
        "message": "Organized optimal ability"
    }, {
        "title": 2,
        "name": "Océane",
        "message": "Balanced bottom-line access"
    }, {
        "title": 2,
        "name": "Laurène",
        "message": "Persevering reciprocal Graphic Interface"
    }, {
        "title": 3,
        "name": "Bécassine",
        "message": "Focused contextually-based orchestration"
    }, {
        "title": 3,
        "name": "Lén",
        "message": "Persevering interactive hub"
    }, {
        "title": 1,
        "name": "Thérèse",
        "message": "Diverse 3rd generation orchestration"
    }, {
        "title": 3,
        "name": "Salomé",
        "message": "Versatile encompassing website"
    }, {
        "title": 2,
        "name": "Hélène",
        "message": "Public-key context-sensitive matrix"
    }, {
        "title": 2,
        "name": "Sòng",
        "message": "Advanced attitude-oriented policy"
    }, {
        "title": 1,
        "name": "Salomé",
        "message": "Expanded national superstructure"
    }, {
        "title": 2,
        "name": "Fèi",
        "message": "Fundamental tertiary hierarchy"
    }, {
        "title": 1,
        "name": "Lén",
        "message": "Inverse real-time benchmark"
    }, {
        "title": 1,
        "name": "Almérinda",
        "message": "Customizable didactic open system"
    }, {
        "title": 3,
        "name": "Lucrèce",
        "message": "Balanced object-oriented function"
    }, {
        "title": 2,
        "name": "Yóu",
        "message": "Re-engineered zero defect support"
    }, {
        "title": 2,
        "name": "Eugénie",
        "message": "Visionary asynchronous policy"
    }]
}


]